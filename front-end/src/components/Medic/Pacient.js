...
export class Pacient extends React.Component {
  ...
  formSubmit(event) {
   ...
   /** superagent.post("/medics/createPacient").set("Content-Type", "application/json").set("token", localStorage.getItem("token"))
      .send({
        ...
        console.log("hola");
        if (err) {
          this.setState({ errorMessage: "Cannot Authenticate Failed" });
          return;
        }
        console.log("Eureka");
